...
abstract class Curve {
     ...
     */
    public static int pointCrossingsForPath(IPathIterator pi,
            double px, double py) {
        if (pi.isDone()) {
            return 0;
        }
        int coords[] = new int[6];
        if (pi.currentSegment(coords) != IPathIterator.SEG_MOVETO) {
     ...
     */
    public static int rectCrossingsForPath(IPathIterator pi,
    ...
    public boolean accumulateCrossings(Crossings c) {
        ...
        if (hitLo) {
            c.record((int) (ystart + .5), (int) (yend + .5), direction);
        }
        return false;
    }
