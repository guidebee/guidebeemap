...
final class Order3 extends Curve {
    ...
    public Order3(double x0, double y0,
        ...
        this.x1 = x1;
        this.y1 = y1;
        xmin = MathEx.min(MathEx.min(x0, x1), MathEx.min(cx0, cx1));
        xmax = MathEx.max(MathEx.max(x0, x1), MathEx.max(cx0, cx1));
        xcoeff0 = x0;
        xcoeff1 = (cx0 - x0) * 3.0;
    ...
    public double TforY(double y) {
        ...
        if (R2 < Q3) {
            ...
            Q = -2.0 * MathEx.sqrt(Q);
            t = refine(a, b, c, y, Q * MathEx.cos(theta / 3.0) - a_3);
            if (t < 0) {
                t = refine(a, b, c, y,
                        Q * MathEx.cos((theta + MathEx.PI * 2.0) / 3.0) - a_3);
            }
            if (t < 0) {
                t = refine(a, b, c, y,
                        Q * MathEx.cos((theta - MathEx.PI * 2.0) / 3.0) - a_3);
    ...
    public double refine(double a, double b, double c,
        ...
        if (false && t >= 0 && t <= 1) {
            ...
            if (yerr > 0 || (verbose && tdiff > 0)) {
                ...
                if (MathEx.abs(target - ylow) < MathEx.abs(target - y) ||
                        MathEx.abs(target - yhi) < MathEx.abs(target - y)) {
    ...
    public int getSegment(double coords[]) {
        ...
        return IPathIterator.SEG_CUBICTO;
    }
