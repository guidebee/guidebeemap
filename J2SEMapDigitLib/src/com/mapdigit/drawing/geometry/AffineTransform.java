...
//--------------------------------- IMPORTS ------------------------------------
import com.mapdigit.drawing.geometry.parser.TransformListParser;
...
public class AffineTransform {
	...
	case (APPLY_SHEAR):
		...
		if (m00 == 1.0 && m11 == 1.0) {
		    this.type = (currentState == APPLY_IDENTITY
				 ? TYPE_IDENTITY
				 : TYPE_TRANSLATION);
		} else {
	...
	case (APPLY_SCALE):
	    ...
	    m11 *= sy;
	    if (m00 == 1.0 && m11 == 1.0) {
		this.state = (currentState &= APPLY_TRANSLATE);
		this.type = (currentState == APPLY_IDENTITY
			     ? TYPE_IDENTITY
			     : TYPE_TRANSLATION);
	    } else {
	...
	case (APPLY_IDENTITY):
	    ...
	    m11 = sy;
	    if (sx != 1.0 || sy != 1.0) {
		this.state = currentState | APPLY_SCALE;
		this.type = TYPE_UNKNOWN;
	    }
	    return;
	}
	...
	case (APPLY_SCALE):
	    ...
	    m10 = m11 * shy;
	    if (m01 != 0.0 || m10 != 0.0) {
		this.state = currentState | APPLY_SHEAR;
	    }
	    this.type = TYPE_UNKNOWN;
	    return;
	case (APPLY_TRANSLATE):
	case (APPLY_IDENTITY):
	    ...
	    m10 = shy;
	    if (m01 != 0.0 || m10 != 0.0) {
		this.state = currentState | APPLY_SCALE | APPLY_SHEAR;
		this.type = TYPE_UNKNOWN;
	    }
	    return;
	}
	...
	case (APPLY_SHEAR | APPLY_SCALE | APPLY_TRANSLATE):
	    ptDst.setLocation(round(x * m00 + y * m01 + m02),
			      round(x * m10 + y * m11 + m12));
	    return ptDst;
	case (APPLY_SHEAR | APPLY_SCALE):
	    ptDst.setLocation(round(x * m00 + y * m01),
                    round(x * m10 + y * m11));
	    return ptDst;
	case (APPLY_SHEAR | APPLY_TRANSLATE):
	    ptDst.setLocation(round(y * m01 + m02),
                    round(x * m10 + m12));
	    return ptDst;
	case (APPLY_SHEAR):
	    ptDst.setLocation(round(y * m01),
                    round(x * m10));
	    return ptDst;
	case (APPLY_SCALE | APPLY_TRANSLATE):
	    ptDst.setLocation(round(x * m00 + m02),
                    round(y * m11 + m12));
	    return ptDst;
	case (APPLY_SCALE):
	    ptDst.setLocation(round(x * m00),
                    round(y * m11));
	    return ptDst;
	case (APPLY_TRANSLATE):
	    ptDst.setLocation(round(x + m02),
                    round(y + m12));
	    return ptDst;
	case (APPLY_IDENTITY):
	...
	int currentState = this.state;
	    ...
	    case (APPLY_SHEAR | APPLY_SCALE | APPLY_TRANSLATE):
		dst.setLocation(round(x * m00 + y * m01 + m02),
				round(x * m10 + y * m11 + m12));
		break;
	    case (APPLY_SHEAR | APPLY_SCALE):
		dst.setLocation(round(x * m00 + y * m01),
                        round(x * m10 + y * m11));
		break;
	    case (APPLY_SHEAR | APPLY_TRANSLATE):
		dst.setLocation(round(y * m01 + m02),
                        round(x * m10 + m12));
		break;
	    case (APPLY_SHEAR):
		dst.setLocation(round(y * m01), round(x * m10));
		break;
	    case (APPLY_SCALE | APPLY_TRANSLATE):
		dst.setLocation(round(x * m00 + m02),
                        round(y * m11 + m12));
		break;
	    case (APPLY_SCALE):
		dst.setLocation(round(x * m00),
                        round(y * m11));
		break;
	    case (APPLY_TRANSLATE):
		dst.setLocation(round(x + m02),
                        round(y + m12));
		break;
	    case (APPLY_IDENTITY):
	...
	case (APPLY_SHEAR | APPLY_SCALE):
	    ...
	    ptDst.setLocation(round((x * m11 - y * m01) / det),
			      round((y * m00 - x * m10) / det));
	    return ptDst;
	case (APPLY_SHEAR | APPLY_TRANSLATE):
	    x -= m02;
	    y -= m12;
	...
	case (APPLY_SCALE):
	    ...
	    return ptDst;
	case (APPLY_TRANSLATE):
	    ptDst.setLocation(round(x - m02), round(y - m12));
	    return ptDst;
	case (APPLY_IDENTITY):
	    ptDst.setLocation(round(x), round(y));
	    return ptDst;
	...
	case (APPLY_SCALE | APPLY_TRANSLATE):
	case (APPLY_SCALE):
	    ptDst.setLocation(round(x * m00), round(y * m11));
	    return ptDst;
	case (APPLY_TRANSLATE):
	case (APPLY_IDENTITY):
	    ptDst.setLocation(round(x), round(y));
     ...
     * tranform parser
     */
    private static final TransformListParser  transformListParser=new TransformListParser();
     ...
     * second floating-point argument.  Note that unlike the copySign method,
     * this method
	...
	m10 = m11;
	m11 = -M0;
	int currentState = rot90conversion[this.state];
